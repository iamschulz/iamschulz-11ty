...
module.exports = async () => {
	...
	const db = await EleventyFetch(`https://api.notion.com/v1/databases/${process.env.NOTION_DEMO_ID}/query`, {
	...
	const posts = db.results.map((result) => ({
		...
		content: result.properties["Description"]?.rich_text.pop()?.plain_text || "",
		...
		screenshots: result.properties["Screenshots"].files.map((image) => image.file.url),
	}));
